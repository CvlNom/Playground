...
const CheckTable = (props) => {
	...
	const calculateResult = (newRows, rowIndex) => {
		...
		if (rows[rowIndex].column5 === 'CRD' || rows[rowIndex].column5 === 'select') {
			...
			let dia = dia2;
			let redH = dieH - dia * (back / 100) - dia * (bear / 100) - dia * (appr / 100);
			let contactH = (dia1 - dia2) / 2 / Math.tan(b);
			const result1 = ((1 / a) * Math.pow(1 + Math.sqrt(1 - a), 2) * Math.sin(b)).toFixed(2); // 예시로 합계를 결과로 설정
			const result2 = (((((dia1 - dia2) / 2) * (1 / Math.tan(b))) / dia2) * 100).toFixed(1); // 예시로 곱셈을 결과로 설정
						...
						{Array.from({ length: 12 }, (_, colIndex) => {
							...
							if (colIndex === 9) {
								cellStyle.backgroundColor =
									row.result1 < 1.6 && row.result1 > 0.1 ? 'lightgreen' : 'rgb(200, 200, 200)';
							}
							if (colIndex === 10) {
								cellStyle.backgroundColor = row.result2 < 100 && row.result2 > 65 ? 'lightgreen' : 'rgb(200, 200, 200)';
							}
							if (colIndex === 11) {
								cellStyle.backgroundColor =
									row.result3 < 70 && row.result2 > 15 ? 'lightgreen' : 'rgba(203, 139, 99, 0.616)';
							}
							if (colIndex === 0) {
								return (
									<td key={colIndex} style={{ width: '45px', backgroundColor: 'lightgray' }}>
										{row.rowNumber}
									</td>
								);
							} else if (colIndex === 1) {
								return (
									<td key={colIndex} style={{ width: '100px', backgroundColor: 'lightgray' }}>
										{props.data[row.rowNumber]} mm
									</td>
								);
							} else if (colIndex === 2) {
								return (
									<td key={colIndex} style={{ width: '80px', backgroundColor: 'lightgray' }}>
								...
								return (
									<td key={colIndex} style={{ width: '70px' }}>
										<input
											...
											onChange={(e) => handleInputChange(rowIndex, `column${colIndex + 1}`, e.target.value)}
										/>
									</td>
								);
							} else if (colIndex === 9) {
